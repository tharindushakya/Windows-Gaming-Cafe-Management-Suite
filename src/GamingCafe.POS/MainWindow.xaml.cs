...
namespace GamingCafe.POS
{
    public partial class MainWindow : Window
    {
        ...
        private void InitializeDatabase()
        {
            ...
            // If the settings explicitly request a provider, prefer it. Otherwise fall back to heuristic.
            var providerSetting = _settings?.DatabaseProvider ?? "Auto";
            if (string.Equals(providerSetting, "Postgres", StringComparison.OrdinalIgnoreCase))
            {
                try
                {
                ...
                {
                    Logger.Log($"Failed to configure Npgsql provider (settings override), falling back to SqlServer: {ex.Message}");
                    optionsBuilder.UseSqlServer(connectionString);
                }
            }
            else if (string.Equals(providerSetting, "SqlServer", StringComparison.OrdinalIgnoreCase))
            {
                optionsBuilder.UseSqlServer(connectionString);
            }
            else
            {
                // Simple heuristic: if the connection string contains Postgres keywords, use Npgsql provider
                var lower = (connectionString ?? string.Empty).ToLowerInvariant();
                if (lower.Contains("host=") || lower.Contains("port=") || lower.Contains("username=") || lower.Contains("user id=") || lower.Contains("database=") || lower.Contains("sslmode="))
                {
                    try
                    {
                        optionsBuilder.UseNpgsql(connectionString);
                    ...
                    {
                        Logger.Log($"Failed to configure Npgsql provider, falling back to SqlServer: {ex.Message}");
                        optionsBuilder.UseSqlServer(connectionString);
                    }
                }
                else
            ...
            try
            {
                ...
                Logger.Log($"[DIAG] Initialized DbConnection. ConnectionString='{conn.ConnectionString ?? "(null)"}', Provider='{_context.Database.ProviderName ?? "(null)"}'");
            }
            catch (Exception ex)
            {
                Logger.Log(ex);
        ...
        private async Task ProcessSale(PaymentMethod paymentMethod)
        {
            ...
            try
            {
                ...
                foreach (var item in _cartItems)
                {
                    ...
                    {
                        UserId = _selectedCustomerId ?? 1, // attribute to selected customer when available
